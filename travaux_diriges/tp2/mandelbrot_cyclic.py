...
bufferFilename = f"outputMandelbrotCyclic/outputMandelbrot{rank:03d}.txt"
...
deb = time.time()
for y in range(rank, height, nbp): #Changer le pas du for pour faire un algorithm de round-robin
    new_y = (y - rank) // nbp #Index utilisé pour la matrice locale
    for x in range(width):
        c = complex(-2. + scaleX*x, -1.125 + scaleY * y)
        convergence[new_y, x] = mandelbrot_set.convergence(c, smooth=True)
fin = time.time()
calc_time = fin - deb
...
if rank == 0:
    ...
    final_image = np.zeros((height, width), dtype=np.double)
    for i in range(nbp):
        block_deb = Nloc * i
        block_fin = Nloc * (i+1)
        final_image[i::nbp, :] = global_array[block_deb:block_fin, :]

    image = Image.fromarray(np.uint8(matplotlib.cm.plasma(final_image)*255))

    fin = time.time()
    out.write(f"Temps de constitution de l'image : {fin-deb}\n")
    image.save("outputMandelbrotCyclic/mandelbrot.png")
    image.show()
