...
if __name__ == '__main__':
    ...
    calcCom = globCom.Split(color, rank)

    calc_rank = calcCom.rank
    calc_nbp = calcCom.size
    ...
    dims_global = init_pattern[0] 
    ny_global, nx_global = dims_global

    nb_workers = nbp - 1
    ny_local = ny_global // nb_workers

    if rank == 0:
        grid = Grille(*init_pattern)
    else:
        grid = Grille((ny_local+2,nx_global), init_pattern=[])

    if rank == 0:
        for i in range(1,nbp):
            i_beg = (i-1)*ny_local
            i_end = i*ny_local
            data_to_send = grid.cells[i_beg:i_end, :]
            globCom.send(data_to_send, dest=i, tag=10)
    else:
        data_received = globCom.recv(source=0, tag = 10)
        grid.cells[1:-1,:] = data_received

    out.write(f"Rank {rank} shape: {grid.cells.shape}\n")
    out.write(f"Rank {rank} Sample:\n{grid.cells}\n")
    #out.flush()
    ...
    while mustContinue:
        ...
        if rank!=0:
            #Ghost Cells
            up_neighbor = (calc_rank-1)%calc_nbp
            down_neighbor = (calc_rank+1)%calc_nbp

            calcCom.Sendrecv(sendbuf=grid.cells[1,:], dest = up_neighbor,
                             recvbuf=grid.cells[-1,:], source = down_neighbor)
            
            calcCom.Sendrecv(sendbuf=grid.cells[-2,:], dest = down_neighbor,
                             recvbuf=grid.cells[0,:], source = up_neighbor)
            ...
            #Transforming the list and the size in numpy arrays
            offset = calc_rank * ny_local * nx_global
            diff_global = []
            min_valid = nx_global
            max_valid = (ny_local + 1) * nx_global

            #Ignore changes made in the ghost cells
            for idx, state in diff:
                if idx < min_valid or idx >= max_valid:
                    continue
                global_idx = (idx - nx_global) + offset
                diff_global.append((global_idx, state))

            diff_np = np.array(diff_global, dtype=np.int32).flatten()
            local_count = np.array([len(diff_np)], dtype=np.int32)

            #Getting how many positions changed in each iteration
            all_counts = np.empty(calc_nbp, dtype=np.int32) if calc_rank == 0 else None
            calcCom.Gather(sendbuf=local_count, recvbuf=all_counts, root=0)

            #Setting parameters for gathering
            recvbuf = None
            if calc_rank == 0:
                total_len = np.sum(all_counts)
                displs = np.insert(np.cumsum(all_counts[:-1]), 0, 0)
                recvbuf_diffs = np.empty(total_len, dtype=np.int32)
                rcv_param = [recvbuf_diffs, (all_counts, displs), MPI.INT32_T]
            else:
                rcv_param = None

            calcCom.Gatherv(sendbuf=diff_np, recvbuf=rcv_param, root=0)

            if calc_rank==0:
                globCom.send(recvbuf_diffs, dest=0, tag=20)
                mustContinue = globCom.recv(source = 0, tag=30)

            mustContinue = calcCom.bcast(mustContinue, root=0)

    

        if rank==0:
            received_diff_flat = globCom.recv(source=1, tag=20)

            t1 = time.time()
            if len(received_diff_flat) > 0:
                diff_formatted = received_diff_flat.reshape(-1, 2)
                grid.change_diff_cells(diff_formatted)
            appli.draw()
            t2 = time.time()
            ...
            globCom.send(mustContinue, dest=1,tag=30)
